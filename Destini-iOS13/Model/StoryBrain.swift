...
struct StoryBrain {
    ...
    {
        didSet {
            self.leftEvent = self.events[currentEvent.choice1Destination]
            self.rightEvent = self.events[currentEvent.choice2Destination]
        }
    }
    
    var leftEvent: StoryEvent
    
    var rightEvent: StoryEvent
    
    init() {
        let root = self.events[0]
        self.currentEvent = root
        self.leftEvent = self.events[root.choice1Destination]
        self.rightEvent = self.events[root.choice2Destination]
    }
}
